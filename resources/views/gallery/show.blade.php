            ...
            <div class="invoice p-3 mb-3 rounded">
                <h2 class="text-left">
                    @if((auth()->user()->role == "Sector HQ" || auth()->user()->role == "CSB 61" || auth()->user()->role == "CSB 64") && auth()->user()->designation != 'Clerk')
                        Event: {{$gallery->title}} - {{\Carbon\Carbon::parse($gallery->date)->format('d-M-Y')}}
                    @else
                ...
                <p class="text-justify">Description: {{$gallery->description}}</p>
                <div class="row">
                    @php
                        ...
                        @else
                            <div class="col-lg-3 col-md-4 col-xs-6 thumb">
                                ...
                                <a class="thumbnail" href="#" data-image-id="" data-toggle="modal" data-title="" data-image="{{\Illuminate\Support\Facades\Storage::url($key)}}" data-target="#image-gallery">
                                    <img class="img-thumbnail" src="{{\Illuminate\Support\Facades\Storage::url($key)}}" style="width: 200px; height: 200px;">
                                </a>

                                @if((auth()->user()->role == "Sector HQ" || auth()->user()->role == "CSB 61" || auth()->user()->role == "CSB 64") && auth()->user()->designation != 'Clerk')
                                @else
                                    <form action="{{route('gallery.destroy', [$gallery->id, 'id' => $key])}}" method="post">
                                        @csrf
                                        @method('DELETE')
